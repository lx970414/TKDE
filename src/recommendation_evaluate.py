import numpy as np
import torch
from sklearn.metrics import precision_recall_curve, roc_auc_score, f1_score, auc, precision_score
...
def recommendation_evaluate(model, true_edges, false_edges):
    """
...
def recommendation_model(model, file_name, feature, A, encode, eval_type, node_matching):
    """
    Recommendation training process using BPR loss
    """
    ...
    for _ in range(1):
        for iter_ in range(100):
            ...
            pos_scores = torch.sum(emb_true_first * emb_true_second, dim=1)
            neg_scores = torch.sum(emb_false_first * emb_false_second, dim=1)
            ...
            # valid_auc, valid_pr = recommendation_evaluate_bpr(final_model, valid_true_data_by_edge, valid_false_data_by_edge)
            # valid_aucs.append(valid_auc)
            # valid_prs.append(valid_pr)

            test_auc, test_pr = recommendation_evaluate_bpr(final_model, testing_true_data_by_edge, testing_false_data_by_edge)
            aucs.append(test_auc)
            prs.append(test_pr)
            
            print("{}\t{:.4f}".format(iter_ + 1, loss.item()))
            print("valid_auc:{:.4f}\t\tvalid_pr:{:.4f}".format(np.mean(valid_aucs), np.mean(valid_prs)))
            print("test_auc:{:.4f}\ttest_pr:{:.4f}".format(np.mean(aucs), np.mean(prs)))

    max_iter_aucs = valid_aucs.index(max(valid_aucs))
    max_iter_prs = valid_prs.index(max(valid_prs))

    return aucs[max_iter_aucs], prs[max_iter_prs]

def recommendation_evaluate_bpr(final_model, true_data_by_edge, false_data_by_edge):
    true_labels = [1] * len(true_data_by_edge)
    false_labels = [0] * len(false_data_by_edge)

    all_edges = true_data_by_edge + false_data_by_edge
    all_labels = true_labels + false_labels

    scores = [torch.sum(final_model[str(edge[0])] * final_model[str(edge[1])]).item() for edge in all_edges]
    auc = roc_auc_score(all_labels, scores)
    precision = precision_score(all_labels, [1 if score >= 0.5 else 0 for score in scores])
    return auc, precision


# def predict_model(model, file_name, feature, A,encode, eval_type, node_matching):
    """
    Recommendation training proces
    """
    ...
    for _ in range(1):
        for iter_ in range(100):
            ...
            # pos_out = np.dot(user_embeds, pos_item_embeds)
            # neg_out = np.dot(user_embeds, pos_item_embeds)
            out_diff = pos_out - neg_out
            lambda_reg = torch.tensor

            pos_out = torch.sum(user_embeds * pos_item_embeds, dim=1)
            neg_out = torch.sum(user_embeds * neg_item_embeds, dim=1)
            loss = torch.mean(nn.functional.softplus(neg_out - pos_out))
            reg_loss = (1/2)*(users_embeds.norm(2).pow(2) + 
                          pos_embeds.norm(2).pow(2) + 
                          neg_embeds.norm(2).pow(2))/float(len(users))
            ...
            for i in range(edge_type_count):
                if eval_type == 'all' or edge_types[i] in eval_type.split(','):
                    train_auc, train_f1, train_pr = recommendation_evaluate(final_model,
                    ...
                    valid_auc, valid_f1, valid_pr = recommendation_evaluate(final_model,
                    ...
                    test_auc, test_f1, test_pr = recommendation_evaluate(final_model,
